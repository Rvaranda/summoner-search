...
function Match({ summonerId, matchDetail, champions }) {
    ...
    useEffect(() => {
        matchDetail.info.participants.forEach(element => {
            if (element.puuid === summonerId) setSummoner(element);
        });

        let created = new Date(matchDetail.info.gameCreation);

        // gameDuration está em segundos, entao multiplica por 1000 para passar para milisegundos
        let dur = new Date(matchDetail.info.gameDuration * 1000)
